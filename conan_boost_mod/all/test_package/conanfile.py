...
class TestPackageConan(ConanFile):
    ...
    def build(self):
        ...
        with tools.vcvars(self.settings) if (
            self.settings.os == "Windows" and self.settings.compiler == "clang"
        ) else tools.no_op():
            cmake = CMake(self)
            cmake.definitions["HEADER_ONLY"] = self.options["boost"].header_only
            if not self.options["boost"].header_only:
                cmake.definitions["Boost_USE_STATIC_LIBS"] = not self.options[
                    "boost"
                ].shared
            ...
            if not self.options["boost"].without_python:
                ...
                cmake.definitions["Python_ADDITIONAL_VERSIONS"] = "{}.{}".format(
            ...
            cmake.definitions["WITH_COROUTINE"] = not self.options[
                "boost"
            ].without_coroutine
            ...
            cmake.definitions["WITH_STACKTRACE"] = not self.options[
                "boost"
            ].without_stacktrace
            cmake.definitions["WITH_STACKTRACE_ADDR2LINE"] = self.deps_user_info[
                "boost"
            ].stacktrace_addr2line_available
            cmake.definitions["WITH_STACKTRACE_BACKTRACE"] = self._boost_option(
                "with_stacktrace_backtrace", False
            )
            if (
                self.options["boost"].namespace != "boost"
                and not self.options["boost"].namespace_alias
            ):
    ...
    def test(self):
        ...
        if not self.options["boost"].without_python:
            with tools.environment_append({"PYTHONPATH": "{}:{}".format("bin", "lib")}):
                self.run(
                    "{} {}".format(
                        self.options["boost"].python_executable,
                        os.path.join(self.source_folder, "python.py"),
                    ),
                    run_environment=True,
                )
        ...
        if not self.options["boost"].without_stacktrace:
            ...
            if (
                str(self.deps_user_info["boost"].stacktrace_addr2line_available)
                == "True"
            ):
                self.run(
                    os.path.join("bin", "stacktrace_addr2line_exe"),
                    run_environment=True,
                )
