...
class BoostConan(ConanFile):
    ...
    def config_options(self):
        ...
        for opt_name in self._configure_options:
            if "without_{}".format(opt_name) not in self.options:
                raise ConanException(
                    "{} has the configure options {} which is not available in conanfile.py".format(
        ...
        if self.settings.compiler.cppstd:
            ...
            if version_cxx11_standard_json:
                if (
                    tools.Version(self.settings.compiler.version)
                    < version_cxx11_standard_json
                ):
                    self.options.without_fiber = True
                    self.options.without_json = True
                    self.options.without_nowide = True
        ...
        if tools.Version(self.version) >= "1.76.0":
            ...
            if self.settings.compiler.cppstd:
                ...
                if min_compiler_version is None:
                    ...
                    tools.Version(self.settings.compiler.version) < min_compiler_version
                ):
                    disable_math()
    ...
    def _stacktrace_addr2line_available(self):
        if (
            self.settings.os in ["iOS", "watchOS", "tvOS"]
    ...
    def configure(self):
        ...
        if self.options.layout == "b2-default":
            self.options.layout = (
                "versioned" if self.settings.os == "Windows" else "system"
            )
    ...
    def validate(self):
            ...
            for lib in [
                ...
                if not self.options.get_safe("without_{}".format(lib)):
                    raise ConanInvalidConfiguration(
                        "Boost '{}' library requires multi threading".format(lib)
                    )
        ...
        if self._stacktrace_addr2line_available:
            if os.path.abspath(str(self.options.addr2line_location)) != str(
                self.options.addr2line_location
            ):
                raise ConanInvalidConfiguration(
            ...
            if not self.options.get_safe("without_{}".format(mod_name), True):
                for mod_dep in mod_deps:
                    if self.options.get_safe("without_{}".format(mod_dep), False):
                        raise ConanInvalidConfiguration(
                            "{} requires {}: {} is disabled".format(
                                mod_name, mod_deps, mod_dep
                            )
                        )
        ...
        if not self.options.get_safe("without_nowide", True):
            ...
            if self.settings.compiler.cppstd:
                ...
                if version_cxx11_standard:
                    ...
                    self.output.warn(
                        "I don't know what the default c++ standard of this compiler is. I suppose it supports c++11 by default.\n"
                        "This might cause some boost libraries not being built and conan components to fail."
                    )

        if not all(
            (self.options.without_fiber, self.options.get_safe("without_json", True))
        ):
                ...
                if self.settings.compiler.cppstd:
                    ...
                    if min_compiler_version is not None:
                        if (
                            ...
                            raise ConanInvalidConfiguration(
                                "Boost.Math requires (boost:)cppstd>=11 (current one is lower)"
                            )
                ...
                if self.settings.compiler.cppstd:
                    ...
                    if min_compiler_version is not None:
                        if (
                            ...
                            raise ConanInvalidConfiguration(
                                "Boost.Wave requires (boost:)cppstd>=11 (current one is lower)"
                            )
    ...
    def _python_version(self):
        ...
        if self.options.python_version and version != self.options.python_version:
            raise ConanInvalidConfiguration(
                "detected python version %s doesn't match conan option %s"
    ...
    def _python_library_dir(self):
        ...
        for prefix in library_prefixes:
            for suffix in library_suffixes:
                for version in versions:
                    candidates.append(
                        "%spython%s%s%s" % (prefix, version, abiflags, suffix)
                    )
        ...
        print("No python library found. Building without.")
        return None
    ...
    def _boost_build_dir(self):
        return os.path.join(
            self.source_folder, self._source_subfolder, "tools", "build"
        )

    def _build_bcp(self):
        folder = os.path.join(
            self.source_folder, self._source_subfolder, "tools", "bcp"
        )
    ...
    def _run_bcp(self):
        with tools.vcvars(
            ...
            with tools.chdir(self.source_folder):
                ...
                command = (
                    "{bcp} {namespace} {alias} "
                    "{boostdir} {libraries} {outdir}".format(
                        bcp=self._bcp_exe,
                        namespace=namespace,
                        alias=alias,
                        libraries=libraries,
    ...
    def build(self):
        ...
        if self.settings.compiler == "apple-clang" or (
            ...
            and tools.Version(self.settings.compiler.version) < 6
        ):
            tools.replace_in_file(
                os.path.join(
                    self.source_folder,
        ...
        use_vcvars = self._is_msvc and not self.settings.compiler.get_safe(
            "toolset", default=""
        )
    ...
    def _build_flags(self):
        ...
        flags.append(
            "--user-config=%s" % os.path.join(self._boost_build_dir, "user-config.jam")
        )
        ...
        if self._is_msvc:
            flags.append(
                "runtime-link=%s"
                % ("static" if "MT" in msvc_runtime_flag(self) else "shared")
            )
            flags.append(
                "runtime-debugging=%s"
                % ("on" if "d" in msvc_runtime_flag(self) else "off")
            )

        # For details https://boostorg.github.io/build/manual/master/index.html
        flags.append(
            "threading=%s" % ("single" if not self.options.multithreading else "multi")
        )
        ...
        if self.settings.os not in ("Android", "Emscripten"):
            try:
                if self._gnu_cxx11_abi:
                    flags.append(
                        "define=_GLIBCXX_USE_CXX11_ABI=%s" % self._gnu_cxx11_abi
                    )

                if self.settings.compiler in ("clang", "apple-clang"):
                    libcxx = {"libstdc++11": "libstdc++",}.get(
        ...
        if self._with_icu:
            ...
            if not self.options["icu"].shared:
                ...
                if self._is_msvc:
                    icu_ldflags = " ".join(
                        "{}.lib".format(l)
                        for l in self.deps_cpp_info["icu"].system_libs
                    )
                else:
                    icu_ldflags = " ".join(
        ...
        if self.options.get_safe("addr2line_location"):
            cxx_flags.append(
                "-DBOOST_STACKTRACE_ADDR2LINE_LOCATION={}".format(
                    self.options.addr2line_location
                )
            )
    ...
    def _cxx(self):
        ...
        if self.settings.compiler == "gcc":
            return (
                tools.which("g++-%s" % compiler_version)
                or tools.which("g++-%s" % major)
                or tools.which("g++")
                or ""
            )
        if self.settings.compiler == "clang":
            return (
                tools.which("clang++-%s" % compiler_version)
                or tools.which("clang++-%s" % major)
                or tools.which("clang++")
                or ""
            )
        return ""
    ...
    def _create_user_config_jam(self, folder):
        ...
        if self._zip_bzip2_requires_needed:
            ...
            def create_library_config(deps_name, name):
                includedir = '"%s"' % self.deps_cpp_info[deps_name].include_paths[
                    0
                ].replace("\\", "/")
        ...
        if not self.options.without_python:
            ...
            if self._python_libraries is None or self.options.without_python_lib:
                ...
                contents += '\nusing python : {version} : "{executable}" : "{includes}" : "{library_dir}" ;'.format(
                    version=self._python_version,
                    executable=self._python_executable,
                    includes=self._python_includes,
        ...
        if self._with_stacktrace_backtrace:
            cppflags += (
                " ".join(
                    "-I{}".format(p)
                    for p in self.deps_cpp_info["libbacktrace"].include_paths
                )
                + " "
            )
            ldflags += (
                " ".join(
                    "-L{}".format(p)
                    for p in self.deps_cpp_info["libbacktrace"].lib_paths
                )
                + " "
            )
    ...
    def package(self):
        ...
        if self.options.header_only:
            self.copy(
                pattern="*", dst="include/boost", src="%s/boost" % self._boost_dir
            )
        ...
        if self._is_msvc and self._shared:
            ...
            for common_lib in common_libs:
                self.output.info(
                    "Unlinking static duplicate library: {}".format(
                        os.path.join(
                            self.package_folder, "lib", "lib{}.lib".format(common_lib)
                        )
                    )
                )
                os.unlink(
    ...
    def package_info(self):
        ...
        if self.options.buildid:
            ...
            self.cpp_info.components["headers"].defines.append(
                "BOOST_LIB_BUILDID=%s" % self.options.buildid
            )
        ...
        if not self.options.header_only:
            ...
            all_detected_libraries = set(
                l[:-4] if l.endswith(".dll") else l for l in tools.collect_libs(self)
            )
            ...
            for module in self._dependencies["dependencies"].keys():
                missing_depmodules = list(
                    ...
                    if self.options.get_safe("without_{}".format(depmodule), False)
                )
            ...
            for incomplete_component in incomplete_components:
                self.output.warn(
                    "Boost component '{0}' is missing libraries. Try building boost with '-o boost:without_{0}'. (Option is not guaranteed to exist)".format(
            ...
            if non_used:
                raise ConanException(
                    "These libraries were built, but were not used in any boost module: {}".format(
                        non_used
                    )
                )
            ...
            if non_built:
                raise ConanException(
                    "These libraries were expected to be built, but were not built: {}".format(
                        non_built
                    )
                )
            ...
            if not self.options.without_stacktrace:
                ...
                if self._stacktrace_addr2line_available:
                    self.cpp_info.components["stacktrace_addr2line"].defines.extend(
                        [
                            'BOOST_STACKTRACE_ADDR2LINE_LOCATION="{}"'.format(
                                self.options.addr2line_location
                            ),
                ...
                if self.settings.os == "Windows":
                    ...
                    tools.is_apple_os(self.settings.os) or self.settings.os == "FreeBSD"
                ):
            ...
            if not self.options.without_python:
                pyversion = tools.Version(self._python_version)
                self.cpp_info.components[
                    "python{}{}".format(pyversion.major, pyversion.minor)
                ].requires = ["python"]
                if not self._shared:
                ...
                self.cpp_info.components[
                    "numpy{}{}".format(pyversion.major, pyversion.minor)
                ].requires = ["numpy"]
